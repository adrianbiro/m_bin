...
find /srv/log/xp/openshift/appdata/smart-prod-applogs -newermt "$(date -d "2 day ago" +"%Y-%m-%d")" ! -newermt "$(date +"%Y-%m-%d")" -name "burda-*" -print0 | while IFS= read -r -d '' file; do if "$(zgrep -q ERROR "${file}")"; then  echo "${file}"; ~/bin/,parseLog4j.py -e 'ERROR' "${file}"; fi; done
